...
- 间距不要求用户理解像素、margin 或 padding：直接拖动选中元素四边，通过页面实时效果调整；调整范围支持 0–640 px，不得在 160 px 处失效。面板只使用“**和旁边元素的距离 / 内部留白**”。
- 选中框左上角显示独立的移动图标；拖动该图标可移动选中元素，并将精确位移写入本轮修改记录。蓝色边缘控制点只负责间距，移动图标只负责位置，避免一个手势承担两种含义。
- 选中框四角显示缩放控制点，可自由调整宽高；“尺寸与位置”提供“设为正方形”和左上/右上/左下/右下容器吸附，适合不熟悉设计工具的用户快速完成布局。
