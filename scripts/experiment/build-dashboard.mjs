...
async function build() {
  ...
  const crossInput = manifest.skills.map(({ id: skillId }) => {
    ...
    const outfitTotal = Number.isFinite(outfit?.effectScore) ? outfit.effectScore : null;
    const cameraTotal = Number.isFinite(camera?.effectScore) ? camera.effectScore : null;
    const scoredInputs = [outfitTotal, cameraTotal].filter(Number.isFinite);
    return { skillId, outfitTotal, cameraTotal, delta: eligible ? camera.effectScore - outfit.effectScore : null, scoredInputCount: scoredInputs.length, combinedEffectScore: scoredInputs.length ? Math.round(scoredInputs.reduce((sum, score) => sum + score, 0) / scoredInputs.length * 10) / 10 : null };
  }).sort((a, b) => b.scoredInputCount - a.scoredInputCount || b.combinedEffectScore - a.combinedEffectScore || a.skillId.localeCompare(b.skillId)).map((row, index) => ({ ...row, overallRank: index + 1 }));
...
fetch('data.json').then(r=>r.json()).then(d=>{
 settings.innerHTML=d.experimentSettings.map(setting=>'<article class="card experiment-setting" data-setting-id="'+esc(setting.id)+'"><h3>'+esc(setting.title)+'</h3><ul>'+setting.items.map(item=>'<li>'+esc(typeof item==='string'?item:item.label+'：'+item.points+' 分')+'</li>').join('')+'</ul>'+(setting.note?'<p class="setting-note">'+esc(setting.note)+'</p>':'')+'</article>').join('');
 ...
 cross.innerHTML=table(['综合排名','技能正式名称','穿搭 Tab 原型效果','相机上传原型效果','综合效果分','分差'],d.crossInput.map(x=>{const outfit=d.results.find(y=>y.skillId===x.skillId&&y.inputId==='outfit-tab');const camera=d.results.find(y=>y.skillId===x.skillId&&y.inputId==='camera-upload');return [x.overallRank,esc(outfit.capabilityName),effectCell(outfit),effectCell(camera),x.combinedEffectScore??'无法计算',x.delta==null?'无法比较':(x.delta>0?'+':'')+x.delta+' 分']}));
