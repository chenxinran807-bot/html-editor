...
右下角“编辑原型”进入编辑模式。选择带虚线框的视觉元素后，可在安全范围内修改文字、颜色、本地图片、位置偏移和宽高尺寸，也可调整显隐、禁用和预览跳转；留空位置或尺寸会恢复原始自适应布局。支持撤销、重做、浏览器本地持久化、修改 JSON 导出及元素级 Agent 评论导出。切回预览后编辑控件隐藏且不可操作，页面恢复正常浏览交互。
